...
// Book Card Component (Extracted for performance)
const BookCard = ({ book, onEdit, onDelete, isFavorite, onToggleFavorite }) => {
  const [showDetails, setShowDetails] = useState(false);
  const bookId = book.id || book._id;
  const displayImage = book.coverImageUrl || book.image || book.coverImage;
  return (
    <div
...
>>>>>>> 8245d72eb88053c66c81b2426eca892d1e0f4818
      ...
      <div className="p-5">
        <h3
          className="font-bold text-lg text-slate-900 dark:text-white mb-1 line-clamp-1"
          title={book.title}
        >
          {book.title}
        </h3>
...
const LibraryBooks = () => {
  ...
  const { data: librariesData, isLoading: librariesLoading } =
    useGetAllLibrariesQuery(tenantName);

  const libraries = useMemo(() => {
    if (!librariesData) return [];
    return Array.isArray(librariesData)
      ? librariesData
  ...
  const books = useMemo(() => {
    if (!booksData) return [];
    return Array.isArray(booksData)
      ? booksData
  ...
  const totalValue = books.reduce(
    (sum, book) => sum + (book.price || 0) * (book.quantity || 0),
    0,
  );
    ...
    () => [
      {
        ...
        header: "Cover",
        render: (item) => {
          const img = item.coverImageUrl || item.image || item.coverImage;
          return (
          <div className="w-10 h-14 rounded overflow-hidden bg-slate-100 dark:bg-slate-700 shadow-sm">
            {img ? (
              <img
                src={img}
                alt=""
      ...
      { key: "quantity", header: "Qty" },
      {
        ...
        render: (item) => (
          <span
            className={`px-2 py-1 rounded-full text-xs font-medium ${item.quantity > 0 ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-400" : "bg-rose-100 text-rose-800 dark:bg-rose-500/20 dark:text-rose-400"}`}
          >
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            ...
            <div className="relative flex-1 lg:flex-none min-w-[200px]">
              ...
              >
                ...
                {libraries.map((lib) => (
                  <option key={lib.id || lib._id} value={lib.id || lib._id}>
                    {lib.name}
                  </option>
                ))}
              </select>
        ...
        {booksError && (
          <div className="text-center py-10 bg-red-50 dark:bg-red-900/20 rounded-2xl border border-red-200 dark:border-red-700">
            ...
            <p className="text-sm text-red-600 dark:text-red-400">
              {booksError.data?.message ||
                "Could not fetch books for the selected library. Please try again."}
            </p>
          </div>
        )}
        ...
        {booksLoading || librariesLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            ...
            <h3 className="text-xl font-semibold text-slate-700 dark:text-slate-300">
              {libraries.length === 0 ? "No libraries found" : "No books found"}
            </h3>
            <p className="text-slate-500 dark:text-slate-400 mt-2">
              {libraries.length === 0
                ? "Please create a library first"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {currentBooks.map((book) => (
                  <BookCard
                    ...
                    isFavorite={favorites.includes(book.id || book._id)}
